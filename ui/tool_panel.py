...
class ToolPanel:
    ...
    def draw(self):
        with dpg.child_window(tag=self.panel_tag, autosize_x=True, autosize_y=True):
            ...
            dpg.add_button(label="Crop & Rotate", tag="crop_mode", callback=self.toggle_crop_mode)
            ...
            with dpg.child_window(tag="crop_panel", autosize_x=True, show=False):
                ...
                # A button to apply the crop/rotate: it collapses the crop panel and triggers update.
                dpg.add_button(label="Apply Crop & Rotate", callback=self.toggle_crop_mode)
    ...
    def toggle_crop_mode(self, sender, app_data, user_data):
        # Toggle the crop mode flag (stored as a Dear PyGui value)
        ...
        if new_val:
            dpg.configure_item("crop_panel", show=True)
        else:
            dpg.configure_item("crop_panel", show=False)
            # Trigger update when crop mode is closed
